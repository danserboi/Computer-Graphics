...
Mesh* Laborator6::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned short> &indices)
{
	...
	// EX 6: dupa interschimbarea pipe 1 cu pipe 3 si trimiterea normalei pe pipe-ul 3 si culorii vertexului pe pipe-ul 1
	// obtin un altfel de colorit pentru cuburile "cube", "box" ramane tot negru deoarece nu are valori pt culoare
...
void Laborator6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
	...
	// BONUS
	// trimitem timpul aplicatiei catre variabila uniforma ElapsedTime
