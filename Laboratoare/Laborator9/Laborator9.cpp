...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		// BONUS 2
		glm::vec3 pos = GetSceneCamera()->transform->GetWorldPosition();
		modelMatrix = glm::rotate(modelMatrix, atan(pos.x/pos.z), glm::vec3(0, 1, 0));
		RenderSimpleMesh(meshes["square"], shaders["ShaderLab9"], modelMatrix, mapTextures["grass"], mapTextures["random"], true);
	}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2, GLboolean mix)
{
	...
	if (texture2)
	{
		...
		//TODO : Send texture uniform value
		GLint texture_2_loc = glGetUniformLocation(shader->program, "texture_2");
		glUniform1i(texture_2_loc, 1);
	}

	GLint mixLoc = glGetUniformLocation(shader->program, "mix");
	glUniform1i(mixLoc, mix);

	// BONUS 1

	GLint earthObjectLoc = glGetUniformLocation(shader->program, "EarthObject");
	if (meshes["sphere"] == mesh) {
		glUniform1i(earthObjectLoc, 1);
	} else {
		glUniform1i(earthObjectLoc, 0);
	}
