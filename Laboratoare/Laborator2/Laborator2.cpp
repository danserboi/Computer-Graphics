...
void Laborator2::Init()
{
	...
	// Create a mesh box using custom data
	{
		vector<VertexFormat> vertices
		{
			...
			VertexFormat(glm::vec3(1, 1,  1), glm::vec3(0.5, 0.5, 1)),
			VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0.1, 1, 0.5)),
			VertexFormat(glm::vec3(1, 0,  0), glm::vec3(0.8, 0.8, 0.5)),
			VertexFormat(glm::vec3(0, 1,  0), glm::vec3(0.2, 0.8, 0.8)),
			VertexFormat(glm::vec3(1, 1,  0), glm::vec3(0.2, 0.2, 0.2))
		};
	...
	// Create a mesh tetrahedron using custom data
	{
		vector<VertexFormat> vertices
		{
			...
			VertexFormat(glm::vec3(4, 0,  0), glm::vec3(0, 0, 1)),
	...
	// Create a mesh square using custom data
	{
		vector<VertexFormat> vertices
		{
			...
			VertexFormat(glm::vec3(0, 4,  4), glm::vec3(1, 1, 0))
		};
...
void Laborator2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube3"], shaders["VertexColor"], glm::vec3(-1.0f, 1.0f, 1.0f), glm::vec3(0.5f));

	RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(1, 1, 1));

	RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(-3, -4, 1));
	
	// Create a mesh circle with the current N value and render it
	{
		vector<VertexFormat> vertices;
		vector<unsigned short> indices;
		// center of circle
		vertices.push_back(VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0, 0, 1)));
		for (int i = 0; i < N; i++) {
			// create vertex
			double x = cos((i * 360 / N) * PI / 180);
			double y = sin((i * 360 / N) * PI / 180);
			vertices.push_back(VertexFormat(glm::vec3(y, 0, x), glm::vec3(0, 0, 1)));
			// create triangle
			if (i < N - 1) {
				indices.push_back(0);
		...
		// render the created mesh
		RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(0, 0, 0));
	}
