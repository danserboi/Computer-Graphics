...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["ShaderLab7"], modelMatrix, BLUE);
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab7"], modelMatrix, GRAY);
	}
...
void Laborator7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
	...
	GLint materialShininessLocation = glGetUniformLocation(shader->program, "material_shininess");
	glUniform1i(materialShininessLocation, materialShininess);

	GLint materialKdLocation = glGetUniformLocation(shader->program, "material_kd");
	glUniform1f(materialKdLocation, materialKd);

	GLint materialKsLocation = glGetUniformLocation(shader->program, "material_ks");
	glUniform1f(materialKsLocation, materialKs);

	GLint objectColorLocation = glGetUniformLocation(shader->program, "object_color");
